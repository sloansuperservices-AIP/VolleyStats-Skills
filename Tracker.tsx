...
export const Tracker: React.FC<TrackerProps> = ({ onBack }) => {
  ...
  // Optimization: Persistent context for live analysis to prevent GC pressure
  ...
  const analyzeLiveStream = async () => {
     ...
     // Optimization: Reuse the same context/canvas for every frame
     if (!analysisCtxRef.current) {
        const canvas = document.createElement('canvas');
        canvas.width = extractWidth;
        canvas.height = extractHeight;
        // willReadFrequently optimizes for toBlob() readback
