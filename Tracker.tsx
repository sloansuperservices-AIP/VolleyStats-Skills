...
export const Tracker: React.FC<TrackerProps> = ({ onBack }) => {
  ...
  const processingCanvasRef = useRef<CanvasRenderingContext2D | null>(null);
  ...
  const analyzeLiveStream = async () => {
     ...
     // Reuse canvas context to avoid creating new canvas/context every frame
     if (!processingCanvasRef.current) {
        const canvas = document.createElement('canvas');
        canvas.width = extractWidth;
        canvas.height = extractHeight;
        processingCanvasRef.current = canvas.getContext('2d', { willReadFrequently: true });
     }

     const ctx = processingCanvasRef.current;
     // Pass the reusable context to extractFrameFromVideo
