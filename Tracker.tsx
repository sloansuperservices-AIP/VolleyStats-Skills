...
export const Tracker: React.FC<TrackerProps> = ({ onBack }) => {
  ...
  const analyzeLiveStream = async () => {
     ...
     // Initialize processing canvas if needed
     if (!processingCtxRef.current) {
        const canvas = document.createElement('canvas');
        // Width/height will be set by extractFrameFromVideo, but good to init
        canvas.width = extractWidth;
        canvas.height = extractHeight;
        processingCtxRef.current = canvas.getContext('2d', { willReadFrequently: true });
     }

     // Reuse the persistent context
     const blob = await extractFrameFromVideo(video, extractWidth, extractHeight, processingCtxRef.current || undefined);
