...
export const Tracker: React.FC<TrackerProps> = ({ onBack }) => {
  ...
  // Optimization: Reuse canvas context for frame extraction
  const extractionContextRef = useRef<CanvasRenderingContext2D | null>(null);
  ...
  const analyzeVideo = async () => {
    ...
    // Use the persistent context if available, otherwise create one (unlikely if loop runs)
    // Actually, for file analysis, we can also reuse a context if we want, but it's separate from live.
    // Let's create one for the file analysis session.
